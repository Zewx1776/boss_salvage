...
local function reset_exploration()
    ...
    --onsole.print("Exploration reset. All areas marked as unexplored.")
end
...
on_update(function()
    ...
    if current_core_time - last_call_time > 0.45 then
        ...
        is_player_on_quest = utils.player_on_quest(get_current_world():get_current_zone_name()) and settings.enabled
        if not is_player_on_quest then
            return
        end

        check_walkable_area()
        settings.is_stuck = check_if_stuck()
        if settings.is_stuck then
